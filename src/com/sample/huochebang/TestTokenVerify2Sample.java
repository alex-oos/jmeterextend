...
public class TestTokenVerify2Sample implements JavaSamplerClient {
	...
	static  int i =1;
	static{
		list.add("fe9404c66cdb746bb4aa3175579e44bb");
		list.add("1d19f2280e5940cb28bc1dd5c2f3f64c");
...
//		list.add("902738a4e66ec88afec92a769708f490");
	...
	public SampleResult runTest(JavaSamplerContext context) {
		...
		//map_data.put("location", "0F94A0341AE81BD0成功3768340");
		...
		try {
			Thread.sleep( 1000 );
			SampleResult sampleResultsub= new HuoCheBangHttpRequest("/manbang/index.php?act=member&op=getTokenVerify&key="+token, "生成令牌", "GET").run( map );

			String response=new String(sampleResultsub.getResponseData());
			System.out.println(response+i);
			FileUtils.writeStringToFile( new File( "D:\\green\\Desktop\\TestTokenVerifySample2.txt" ) ,new String( sampleResultsub.getResponseData())+i+"\n" ,"UTF-8",true);
			JsonObject jsonObject=new JsonParser().parse(response).getAsJsonObject();
			vat_hash=jsonObject.get("datas").getAsJsonObject().get("token_verify").getAsString();
		} catch (Exception e) {
			e.printStackTrace();
		}
		...
		Map map_data1 =new HashMap<>();

		map_data1.put( "key",token );
		map_data1.put( "token_verify", vat_hash);

		map1.put("data", map_data1);

		Map map_head1 =new HashMap<>();

		map1.put("head", map_head1);
		new Thread(){
			@Override
			public void run() {
				...
				try {
					...
					SampleResult sampleResultsub1= new HuoCheBangHttpRequest("/manbang/index.php?act=member&op=testTokenVerify", "测试令牌", "POST").run( map1 );
					System.out.println(new String( sampleResultsub1.getResponseData() ));
					FileUtils.writeStringToFile( new File( "D:\\green\\Desktop\\one.txt" ) ,new String( sampleResultsub1.getResponseData() )+i+"\n" ,"UTF-8",true);
		...
		new Thread(){
			...
			public void run() {
				try {
					SampleResult sampleResultsub2= new HuoCheBangHttpRequest("/manbang/index.php?act=member&op=testTokenVerify", "测试令牌", "POST").run(map1);
					System.out.println(new String( sampleResultsub2.getResponseData() ));
					FileUtils.writeStringToFile( new File( "D:\\green\\Desktop\\TestTokenVerifySample2.txt" ) ,new String( sampleResultsub2.getResponseData() )+i+"\n" ,"UTF-8",true);
				} catch (Exception e) {
					e.printStackTrace();
				}
			}
	...
	public static void main(String[] args) {
			...
			SampleResult sampleResult = new TestTokenVerify2Sample().runTest( null );
