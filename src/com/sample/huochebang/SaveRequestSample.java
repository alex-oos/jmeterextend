...
public class SaveRequestSample implements JavaSamplerClient {
	...
	public SampleResult runTest(JavaSamplerContext context) {
		...
		Map map1_data =new HashMap<>();
		
		//map_data.put("location", "0F94A0341AE81BD0成功3768340");
	
		
		map1.put("data", map1_data);
	
		Map map1_head =new HashMap<>();
		
		//map_head.put("token", token);
		map1.put("head", map1_head);
		String token_verify="";
		try {
			SampleResult sampleResultsub= new HuoCheBangHttpRequest("/manbang/index.php?act=member&op=getTokenVerify&key="+token, "生成令牌", "GET").run(map1);
			JsonObject jsonObject=(JsonObject) new JsonParser().parse(new String(sampleResultsub.getResponseData()));
			
			token_verify=jsonObject.get("datas").getAsJsonObject().get("token_verify").getAsString();
		} catch (Exception e) {
			e.printStackTrace();
		}
		System.out.println(token_verify);
		Map map =new HashMap<>();
		
		Map map_data =new HashMap<>();
		
		map_data.put("sid", "3768340");
		map_data.put("st", "0F94A0341AE81BD0");
		map_data.put("token_verify", token_verify);
		map_data.put("key", "");
		...
		try {
			SampleResult sampleResultsub= new HuoCheBangHttpRequest("/manbang/index.php?act=log&op=saveRequest", "保存前端对接日志接口", "POST").run(map);
	...
	public static void main(String[] args) {
		SampleResult sampleResult=new SaveRequestSample().runTest(null);
