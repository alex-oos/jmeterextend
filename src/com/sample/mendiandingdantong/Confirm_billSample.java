...
public class Confirm_billSample implements JavaSamplerClient {

	static List list =new ArrayList();
	static List ob_idlist =new ArrayList();
	static int count=0;
	static{
		
		list.add("0c287ffafc54eec61dd6fc04a01160f1");
		
		String sql="select shopnc_order_bill.ob_id FROM `shopnc_order_bill` WHERE shopnc_order_bill.ceshi=1 AND shopnc_order_bill.ob_state=1 LIMIT 200000";
		
		try {
			ob_idlist=db.selectbysql(sql);
		} catch (Exception e) {
			e.printStackTrace();
		}
...
//		list.add("8d17db782f7fe20efc8d499ce8b552bc");
	...
	public SampleResult runTest(JavaSamplerContext context) {
	    ...
	    String ob_id="";
	    synchronized (lock) {
	    	ob_id =Long.toString((long)((Map)ob_idlist.get(i)).get("ob_id")); 
	    	i++;
		}
	    ...
	    //确认账单
  		...
  		try {
			SampleResult sampleResultsub= new AppHttpRequest("/orderapi/v1/chain_order_bill/confirm_bill", "确认账单", "POST").run(map);
