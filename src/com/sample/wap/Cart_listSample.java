...
/**
 * @author lijiang
 * @date 2019/1/8 - 下午 2:07
 */
public class Cart_listSample implements JavaSamplerClient {
...
//        System.out.println("token为"+key+"q为"+q);
        q++;
        Map map_data=new HashMap<>();
        map_data.put("data", "key="+key);
        try {
            SampleResult sampleResult=new HttpRequest2("/index.php?act=member_cart&op=cart_list" , "购物车列表", "POST").run(map_data);
            return sampleResult;
        } catch (Exception e) {
