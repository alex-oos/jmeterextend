...
/**
 * @author lijiang
 * @date 2019/1/8 - 下午 2:47
 */
public class GetstoregradeSample implements JavaSamplerClient {
    static List tokenlist =new ArrayList();
    static List store_idlist= new ArrayList();
    static  int store_idcount = 0;
    static int count=0;
//    static  int q =0;
    static {
    ...
    store_idlist.add("55");
        ...
        store_idcount=store_idlist.size();
    }
    @Override
    ...
    public SampleResult runTest(JavaSamplerContext javaSamplerContext) {
        ...
        int c =(int) (Math.random()*store_idcount);
        String store_id =(String) store_idlist.get(c);
        int m =(int) (Math.random()*store_idcount);
        String store_id1 =(String) store_idlist.get(m);

        Map map_data=new HashMap<>();
        map_data.put("data", "");
        try {
            SampleResult sampleResult=new HttpRequest2("/mobile/index.php?act=member_buy&op=getStoreGrade&key="+key+
                    "&storeIdStr="+store_id+","+store_id1, "获取店铺等级", "GET").run(map_data);
...
//            SampleResult sampleResult =new GetstoregradeSample().runTest(null);
