...
/**
 * @author lijiang
 * @date 2019/1/8 - 下午 3:01
 */
public class Cart_batchaddSample implements JavaSamplerClient {
    ...
    public SampleResult runTest(JavaSamplerContext javaSamplerContext) {
        ...
        int  amount2=(int)(Math.random()*8);
        String key =(String) tokenlist.get(index);
        String goods_id =(String) goodidlist.get((int) (Math.random()*goodidcount));
        String goods_id1 =(String) goodidlist.get((int) (Math.random()*goodidcount));
        String goods_id2 =(String) goodidlist.get((int) (Math.random()*goodidcount));
        String goods_id3 =(String) goodidlist.get((int) (Math.random()*goodidcount));
        String data="key="+key+"&cartlist="+goods_id+","+amount+"|"+goods_id1+","+amount1+"|"+goods_id2+","+amount2;

        Map map_data=new HashMap<>();
        map_data.put("data",data);
        try {
            SampleResult sampleResult=new HttpRequest2("/mobile/index.php?act=member_cart&op=cart_batchadd", "批量加入购物车", "POST").run(map_data);
            return sampleResult;
        } catch (Exception e) {
...
//            SampleResult sampleResult =new Cart_batchaddSample().runTest(null);
