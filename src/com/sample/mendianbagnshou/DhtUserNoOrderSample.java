...
public class DhtUserNoOrderSample implements JavaSamplerClient {
	...
	public SampleResult runTest(JavaSamplerContext context) {
	    ...
	    //订货通用户登录门店帮 在未填写认证资料时 不能生成订单
  		...
  		map_orderdhtUserNoOrder_head.put("token", token);
  		map_orderdhtUserNoOrder.put("head", map_orderdhtUserNoOrder_head);

		try {
			SampleResult sampleResultsub= new HttpRequest("/chainapi/v1/member_buy/dhtUserNoOrder", "订货通用户认证", "POST").run(map_orderdhtUserNoOrder);
...
//		SampleResult dhtUserNoOrderSample=new DhtUserNoOrderSample().runTest(null);
//	}
